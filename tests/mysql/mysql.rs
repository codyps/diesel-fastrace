...
use diesel_fastrace::install_default_mysql_instrumentation as install_default;
fn instrumentation(url: &str) -> FastraceInstrumentation {
    FastraceInstrumentation::mysql(url)
}
fn database_url() -> String {
    std::env::var("DIESEL_FASTRACE_TEST_MYSQL_URL").expect("set DIESEL_FASTRACE_TEST_MYSQL_URL")
}
